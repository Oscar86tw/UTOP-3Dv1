...
function buildRelationshipSnapshot(){
 ...
 return {version:'5.1.3.64',generatedAt:new Date().toISOString(),projectName:document.title||'UTOP-3D',items,wires:state.wires.map(w=>({...w})),shortcuts:(state.shortcuts||[]).map(x=>({...x}))};
}
