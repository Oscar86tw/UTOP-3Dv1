...
loopdetector:{name:'地感檢知器',assetId:'LANE-LOOP-DETECTOR-001',defaults:{rotation:0,mode:'PRESENCE',vehicle:false,pulse:false,fault:false,ready:true,sensitivity:4,frequency:'MID',contact:'NO',showLabel:true},di:[['DI1','LOOP_INPUT｜線圈偵測','vehicle'],['DI2','CLEAR｜車輛離開','clear'],['DI3','TEST_PULSE｜脈衝測試','pulse'],['DI4','LOOP_FAULT｜線圈故障','fault'],['DI5','RESET｜重新校正','reset']],do:[['DO1','PRESENCE_OUT｜車輛存在','presence'],['DO2','PULSE_OUT｜脈衝輸出','pulseOut'],['DO3','LOOP_FAULT｜線圈故障','faultOut'],['DO4','READY｜設備正常','ready']]},
 ...
 signal2way:{name:'雙向號誌主機',assetId:'SIGNAL-2WAY-001',defaults:{rotation:0,mode:'ALL_GREEN',intervalSeconds:10,remaining:10,auto:false,state:'READY',showLabel:true},di:[['DI1','LANE_A_CALL｜A向需求','laneA'],['DI2','LANE_B_CALL｜B向需求','laneB'],['DI3','ALL_GREEN｜全綠','allGreen'],['DI4','AUTO｜自動交換','auto'],['DI5','RESET｜復歸','reset']],do:[['DO1','LANE_A_GREEN｜A向綠燈','laneAGreen'],['DO2','LANE_A_RED｜A向紅燈','laneARed'],['DO3','LANE_B_GREEN｜B向綠燈','laneBGreen'],['DO4','LANE_B_RED｜B向紅燈','laneBRed'],['DO5','AUTO_ACTIVE｜自動模式','autoActive']]},
 signal3way:{name:'三向號誌主機',assetId:'SIGNAL-3WAY-001',defaults:{rotation:0,mode:'ALL_GREEN',intervalSeconds:10,remaining:10,auto:false,state:'READY',showLabel:true},di:[['DI1','LANE_A_CALL｜A向需求','laneA'],['DI2','LANE_B_CALL｜B向需求','laneB'],['DI3','LANE_C_CALL｜C向需求','laneC'],['DI4','ALL_GREEN｜全綠','allGreen'],['DI5','AUTO｜自動交換','auto'],['DI6','RESET｜復歸','reset']],do:[['DO1','LANE_A_GREEN｜A向綠燈','laneAGreen'],['DO2','LANE_B_GREEN｜B向綠燈','laneBGreen'],['DO3','LANE_C_GREEN｜C向綠燈','laneCGreen'],['DO4','ALL_RED｜全紅狀態','allRedStatus'],['DO5','AUTO_ACTIVE｜自動模式','autoActive']]},
};
...
function startSignalAuto(item){
 ...
 p.auto=true;
 const modes=item.type==='signal3way'?['LANE_A','LANE_B','LANE_C']:['LANE_A','LANE_B'];
 let idx=0;
 let cycleStart=performance.now();
 let lastShown=seconds;
 p.mode=modes[idx];p.remaining=seconds;p.state='TIMING';
 rebuildItemGeometryOnly(item,{reason:'signal-auto-start'});propagate(item);
 item.runtime.signalTimer=setInterval(()=>{
  const elapsed=(performance.now()-cycleStart)/1000;
  p.remaining=Math.max(0,seconds-elapsed);
  const shown=Math.ceil(p.remaining);
  if(shown!==lastShown){lastShown=shown;rebuildItemGeometryOnly(item,{reason:'signal-tick'});}
  if(p.remaining<=0){
   idx=(idx+1)%modes.length;p.mode=modes[idx];p.remaining=seconds;cycleStart=performance.now();lastShown=seconds;
   rebuildItemGeometryOnly(item,{reason:'signal-switch'});propagate(item);
  }
  if(item.id===state.selectedId){renderQuick(item);renderInspector();}
 },100);
}
...
function input(i,key){
 ...
 if(i.type==='barrier'){
   ...
   if(key==='allGreen'){cancelControlRuntime(i);p.auto=false;p.mode='ALL_GREEN';p.remaining=Math.max(1,Number(p.intervalSeconds)||10);}
   if(key==='auto'){startSignalAuto(i);}
   if(key==='reset'){cancelControlRuntime(i);p.auto=false;p.mode='ALL_GREEN';p.state='READY';}
   rebuild(i);
...
function renderInspector(){
 ...
 if(state.activeTab==='properties'){
   ...
   :i.type==='signal2way'?[['laneA','A向綠燈'],['laneB','B向綠燈'],['allGreen','全綠'],['auto','自動交換'],['reset','復歸']]
   :i.type==='signal3way'?[['laneA','A向綠燈'],['laneB','B向綠燈'],['laneC','C向綠燈'],['allGreen','全綠'],['auto','自動交換'],['reset','復歸']]
