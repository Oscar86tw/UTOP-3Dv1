...
function renderShortcutList(){
 ...
 box.innerHTML=state.shortcuts.length?state.shortcuts.map((entry,index)=>`<article class="shortcut-row"><kbd>${shortcutComboLabel(entry.combo||entry.key)}</kbd><div><strong>${shortcutDescription(entry)}</strong><small>${entry.ioType==='di'?'直接執行設備輸入動作':'模擬 DO 訊號脈衝並送往已接線設備'}</small></div><button data-shortcut-remove="${index}" type="button">刪除</button></article>`).join(''):'<div class="cloud-empty">尚未設定快捷鍵。</div>';
...
$('shortcutAdd')?.addEventListener('click',()=>{
 const key=$('shortcutKey')?.dataset.key||'';const deviceId=$('shortcutDevice')?.value||'';const ioType=$('shortcutIoType')?.value||'di';const port=$('shortcutPort')?.value||'';
 ...
 state.shortcuts.push({combo:key,key,deviceId,ioType,port});renderShortcutList();markDirty();
 $('shortcutKey').value='';$('shortcutKey').dataset.key='';
});
