// UTOP-3D V5.1.3.37 - Cloud save folder creation UI + Drive JSON storage
const SPREADSHEET_ID = '1lmEqbYals_uBIOrQFDupYR2hs0iks5zEJM6FTOuOwP0';
const CONFIG_SHEET_NAME = '工作表1';
const EXEC_CACHE_KEY = 'utop3d_google_exec_url_v51337';
const LAST_FILE_NAME_KEY = 'utop3d_last_google_file_name';
const CLOUD_LOCKS_KEY = 'utop3d_cloud_project_locks_v51337';
...
async function saveGoogleProject() {
  ...
  try {
    ...
    const result = await postBridge('saveProject', {
      ...
      project,
      data: project,
      version: project?.version || '5.1.3.37'
    });
...
async function refreshGoogleProjects(options = {}) {
  $('googleProjectList').innerHTML = '<div class="cloud-empty">正在同步 Google Drive 與雲端索引…</div>';
  try {
    // 每次開啟雲端清單先重新讀 A2，確保使用最新部署網址。
    await resolveAppsScriptUrl(true);

    // V5.1.3.36：Drive 是真實來源。先掃描 Drive，再讀 Projects。
    ...
    if (added || folders || missing) {
      showToast(`雲端索引同步完成：新增專案 ${added}、資料夾 ${folders}${missing ? `、缺檔 ${missing}` : ''}`);
    }
