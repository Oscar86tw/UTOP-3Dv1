...
/* V5.1.3.59 四種模組關聯視圖：提供目前專案的唯讀快照 */
function buildRelationshipSnapshot(){
 ...
 return {version:'5.1.3.59',generatedAt:new Date().toISOString(),projectName:document.title||'UTOP-3D',items,wires:state.wires.map(w=>({...w})),shortcuts:(state.shortcuts||[]).map(x=>({...x}))};
}
function persistRelationshipSnapshot(){
 const snapshot=buildRelationshipSnapshot();
 try{localStorage.setItem('utop3d.relationship.snapshot',JSON.stringify(snapshot));}catch(error){console.warn('[UTOP] 關聯視圖快照無法寫入 localStorage',error);}
 return snapshot;
}
window.UTOP_RELATIONSHIP_API=Object.freeze({getSnapshot:()=>buildRelationshipSnapshot(),refreshSnapshot:()=>persistRelationshipSnapshot()});
function openRelationshipPage(path){
 persistRelationshipSnapshot();
 const win=window.open(path,'_blank');
 if(!win)alert('瀏覽器阻擋了新分頁，請允許此網站開啟分頁。');
}
