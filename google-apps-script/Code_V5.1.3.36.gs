/*******************************************************
 * UTOP-3D Cloud Storage Backend V5.1.3.36
...
const CONFIG = {
  ...
  PROJECT_HEADERS: [
    ...
    'createdAt', 'updatedAt', 'note', 'legacySourceId', 'driveStatus'
  ],
  ...
  try {
    ...
    switch (action) {
      ...
      case 'legacyStatus':
        result = legacyStatus();
        break;
      case 'driveIndexStatus':
        result = driveIndexStatus();
        break;
      default:
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'syncDriveIndex': result = syncDriveIndex(p); break;
      default:
...
function listProjects(p) {
  ...
  const projects = rows.map(r => ({
    ...
    driveStatus: r.driveStatus || (r.driveFileId ? 'OK' : 'LEGACY'),
...
/* =====================================================
   V5.1.3.36 Drive 掃描 / Projects 索引重建
===================================================== */

function driveIndexStatus() {
  const scan = scanDriveJsonFiles_(rootFolder_(), '', true, [], []);
  const projects = sheetToObjects(getSheet(CONFIG.PROJECT_SHEET));
  const indexed = new Set(projects.map(r => String(r.driveFileId || '')).filter(Boolean));
  const missingIndex = scan.files.filter(f => !indexed.has(f.driveFileId));
  return {
    success: true,
    driveJsonCount: scan.files.length,
    indexedProjectCount: projects.filter(r => !toBoolean(r.deleted)).length,
    missingIndexCount: missingIndex.length,
    missingIndexFiles: missingIndex.slice(0, 50)
  };
}

function syncDriveIndex(p) {
  const recursive = p.recursive !== false && String(p.recursive || 'true') !== 'false';
  const createFolderIndex = p.createFolderIndex !== false && String(p.createFolderIndex || 'true') !== 'false';
  const markMissing = p.markMissing !== false && String(p.markMissing || 'true') !== 'false';

  const projectSheet = getSheet(CONFIG.PROJECT_SHEET);
  const folderSheet = getSheet(CONFIG.FOLDER_SHEET);
  let folderRows = sheetToObjects(folderSheet);

  const byDriveFolderId = new Map();
  folderRows.forEach(r => {
    const id = String(r.driveFolderId || '').trim();
    if (id && !toBoolean(r.deleted)) byDriveFolderId.set(id, r);
  });

  const folderChanges = [];
  const scan = scanDriveJsonFiles_(rootFolder_(), '', recursive, [], folderChanges);
  let addedFolders = 0;

  if (createFolderIndex) {
    // 依深度排序，確保父資料夾先建立索引。
    folderChanges.sort((a, b) => a.depth - b.depth);
    for (const f of folderChanges) {
      if (byDriveFolderId.has(f.driveFolderId)) continue;
      let parentFolderId = '';
      if (f.parentDriveFolderId && f.parentDriveFolderId !== UTOP_DRIVE_ROOT_FOLDER_ID) {
        const parentRec = byDriveFolderId.get(f.parentDriveFolderId);
        parentFolderId = parentRec ? String(parentRec.folderId || '') : '';
      }
      const folderId = generateId('folder');
      const now = new Date();
      appendObject(folderSheet, {
        folderId,
        ...
        locked: false,
        deleted: false,
        sortOrder: getNextFolderSortOrder(sheetToObjects(folderSheet)),
        createdAt: f.createdAt || now,
        updatedAt: f.updatedAt || now,
        note: 'V5.1.3.36 由 Drive 自動建立索引'
      });
      const rec = {
        folderId,
        folderName: f.folderName,
      ...
      byDriveFolderId.set(f.driveFolderId, rec);
      addedFolders++;
    }
  }

  let projects = sheetToObjects(projectSheet);
  const byDriveFileId = new Map();
  projects.forEach(r => {
    const id = String(r.driveFileId || '').trim();
    if (id) byDriveFileId.set(id, r);
  });
  ...
  for (const f of scan.files) {
    ...
    const existing = byDriveFileId.get(f.driveFileId);
    const logicalFolder = byDriveFolderId.get(f.parentDriveFolderId);
    const folderId = logicalFolder ? String(logicalFolder.folderId || '') : '';
    const cleanName = String(f.fileName || '').replace(/\.json$/i, '');
    ...
    // Drive 檔存在：修正 URL/檔名/資料夾與狀態，但保留鎖定。
    const rowNumber = existing._rowNumber;
    updateRecord(projectSheet, rowNumber, {
      projectName: cleanName || existing.projectName,
      folderId,
      fileUrl: f.fileUrl,
      fileSize: f.fileSize,
      updatedAt: f.updatedAt,
  ...
  if (markMissing) {
    projects = sheetToObjects(projectSheet);
    for (const r of projects) {
      const driveFileId = String(r.driveFileId || '').trim();
      if (!driveFileId || toBoolean(r.deleted)) continue;
      if (!seenFileIds.has(driveFileId)) {
        updateRecord(projectSheet, r._rowNumber, { driveStatus: 'MISSING', note: 'Drive 找不到實體 JSON；索引保留未刪除' });
        missingProjects++;
      }
    }
  }

  addLog('SYNC_DRIVE_INDEX', 'system', UTOP_DRIVE_ROOT_FOLDER_ID, 'Google Drive', 'SUCCESS',
    'Drive JSON=' + scan.files.length + ', 新增Projects=' + addedProjects + ', 新增Folders=' + addedFolders + ', 缺檔=' + missingProjects);
...
function scanDriveJsonFiles_(folder, parentDriveFolderId, recursive, files, folders, depth) {
  depth = Number(depth || 0);
  const currentDriveFolderId = folder.getId();
  ...
  while (fileIter.hasNext()) {
    ...
    files.push({
      ...
      fileName: name,
      fileUrl: file.getUrl(),
      parentDriveFolderId: currentDriveFolderId,
      fileSize: file.getSize(),
  ...
  if (!recursive) return { files, folders };

  const folderIter = folder.getFolders();
  while (folderIter.hasNext()) {
    const child = folderIter.next();
    folders.push({
      driveFolderId: child.getId(),
      folderName: child.getName(),
      parentDriveFolderId: currentDriveFolderId,
      depth: depth + 1,
      createdAt: child.getDateCreated(),
      updatedAt: child.getLastUpdated()
    });
    scanDriveJsonFiles_(child, currentDriveFolderId, recursive, files, folders, depth + 1);
  }
