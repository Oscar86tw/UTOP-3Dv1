...
========================================================= */
const SHORTCUT_RESERVED_KEYS=new Set(['Escape','F5','F11','Tab','CapsLock','Shift','Control','Alt','Meta']);
...
function executeShortcut(entry){
 ...
 window.UTOP_DEBUG?.record?.('ACTION','Shortcut',`快捷鍵 ${shortcutKeyLabel(entry.key)}：${shortcutDescription(entry)}`);
}
...
$('shortcutKey')?.addEventListener('keydown',event=>{
 ...
 if(event.ctrlKey||event.altKey||event.metaKey||SHORTCUT_RESERVED_KEYS.has(event.key))return;
 event.currentTarget.value=shortcutKeyLabel(normalizeShortcutKey(event.key));
 event.currentTarget.dataset.key=normalizeShortcutKey(event.key);
});
...
$('shortcutAdd')?.addEventListener('click',()=>{
 ...
 state.shortcuts=state.shortcuts.filter(x=>x.key!==key);
 state.shortcuts.push({key,deviceId,ioType,port});renderShortcutList();markDirty();
...
document.addEventListener('keydown',event=>{
 ...
 const key=normalizeShortcutKey(event.key);const entry=state.shortcuts.find(x=>x.key===key);if(!entry)return;
 event.preventDefault();executeShortcut(entry);
});
