...
function vehicleCanPassOpenShutter(vehicle,vt,shutter,st){
 ...
 const progress=Math.max(0,Math.min(1,Number(p.doorProgress)||0));
 const visibleDoorHeight=Math.max(Math.min(.16,Math.max(.065,openingHeight/22)),(1-progress)*openingHeight);
 ...
 const vehicleHalfWidth=vehicleWorldHalfExtents(vehicle,vt).x;
 // 車體必須位於左右門柱之間，且目前門片淨空高於車輛。
 const insideOpening=Math.abs(lp.x)+vehicleHalfWidth <= openingWidth/2-.02;
 const vehicleTop=(Number(vt.y)||0)+vd.height;
 const shutterBase=Number(st.y)||0;
 const enoughHeight=(vehicleTop-shutterBase) <= clearHeight-.02;
