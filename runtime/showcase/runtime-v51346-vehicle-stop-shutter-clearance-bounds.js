...
window.addEventListener('keydown',e=>{
 ...
 if(!isDrivableVehicle(item))return;
 ...
 if(state.activeTab==='properties'){
   ...
   if(i.type==='barrier'){
    ...
    if(isDrivableVehicle(i)){
     ...
     box.innerHTML=`<div class="form-grid"><section class="field-group"><h3>車輛行駛參數</h3><label>車輛種類<input value="${d.name}" disabled></label><label class="checkbox-field"><input id="vehicleDriveEnabled" type="checkbox" ${p.driveEnabled!==false?'checked':''}> 開啟行駛控制</label><label>速度（m/s）<input id="vehicleDriveSpeed" type="number" min="0.1" max="30" step="0.1" value="${Number(p.driveSpeed||3)}"></label><label>轉向速度（°/s）<input id="vehicleSteerRate" type="number" min="5" max="180" step="1" value="${Number(p.steerRate||55)}"></label><small>方向鍵／WASD：按住前進／後退與轉向，放開前進／後退即停止；Space：停止。車輛進入地感線圈或遮斷紅外線時會自動觸發感應模組。</small></section><section class="field-group"><h3>駕駛控制</h3><div class="button-row"><button data-input="driveForward">前進</button><button data-input="driveReverse">後退</button><button data-input="driveLeft">左轉</button><button data-input="driveStraight">回正</button><button data-input="driveRight">右轉</button><button data-input="driveStop">停止</button></div><small>目前：${p.moving?(p.driveDirection===-1?'倒車':'前進'):'停止'}｜轉向：${p.steerInput<0?'左':p.steerInput>0?'右':'直行'}</small></section></div>`;
