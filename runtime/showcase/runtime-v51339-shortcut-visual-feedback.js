...
function renderInspector(){
 ...
 if(state.activeTab==='properties'){
   ...
   :i.type==='signal2way'?[['laneA','A向動作'],['laneB','B向動作'],['allGreen','全綠待命'],['auto','開啟自動待命'],['reset','復歸']]
   :i.type==='signal3way'?[['laneA','A向動作'],['laneB','B向動作'],['laneC','C向動作'],['allGreen','全綠待命'],['auto','開啟自動待命'],['reset','復歸']]
...
function renderQuick(i=get(state.selectedId)){
 ...
 if(i.type==='barrier')$('quickContent').innerHTML=`<div class="quick-buttons"><button data-q="open">開啟</button><button data-q="stop">停止</button><button data-q="close">關閉</button><button data-q="reset">復歸</button></div><div class="status-box">${statusText(i)}</div>`;
   ...
   $('quickContent').innerHTML=`<div class="signal-quick-seconds"><label>動作秒數 <input id="quickSignalSeconds" type="number" min="1" max="999" step="1" value="${Math.max(1,Number(i.params.intervalSeconds)||10)}"> 秒</label></div><div class="quick-buttons"><button data-q="laneA">A向動作</button><button data-q="laneB">B向動作</button>${c}<button data-q="allGreen">全綠待命</button><button data-q="auto">開啟自動待命</button><button data-q="reset">復歸</button></div><div class="status-box">${statusText(i)}</div>`;
...
function shortcutComboLabel(combo){
 return String(combo||'').split('+').map(part=>shortcutKeyLabel(part)).join(' + ');
}
function getShortcutDefinition(item,ioType){
 const d=defs[item?.type];
 return ioType==='do'?(d?.do||[]):(d?.di||[]);
}
function renderShortcutPortOptions(){
 ...
 port.innerHTML=entries.map(x=>`<option value="${x[2]}">${x[0]}｜${x[1]}</option>`).join('');
}
function renderShortcutDeviceOptions(){
 const device=$('shortcutDevice');if(!device)return;
 const current=device.value;
 device.innerHTML=state.items.map(i=>`<option value="${i.id}">${String(i.name||defs[i.type]?.name||i.type).replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]))}</option>`).join('');
...
function shortcutDescription(entry){
 ...
 return `${item?.name||'設備已刪除'}｜${entry.ioType.toUpperCase()} ${def?.[1]||entry.port}`;
}
function shortcutDisplayInfo(entry){
 const item=get(entry.deviceId);
 const def=getShortcutDefinition(item,entry.ioType).find(x=>x[2]===entry.port);
 return {
  item,
  title:String(entry.title||entry.itemName||'快捷觸發').trim()||'快捷觸發',
  io:entry.ioType.toUpperCase(),
  functionName:def?.[1]||entry.port,
  key:shortcutComboLabel(entry.combo||entry.key)
 };
}
function makeShortcutFeedbackSprite(lines){
 const canvas=document.createElement('canvas');canvas.width=900;canvas.height=300;
 const ctx=canvas.getContext('2d');
 ctx.clearRect(0,0,canvas.width,canvas.height);
 ctx.fillStyle='rgba(6,23,33,.92)';ctx.strokeStyle='rgba(255,255,255,.9)';ctx.lineWidth=5;
 const r=28,x=10,y=10,w=880,h=280;
 ...
 ctx.fillStyle='#ffffff';ctx.font='bold 54px sans-serif';ctx.fillText(lines[0]||'',450,70);
 ctx.fillStyle='#ffe082';ctx.font='bold 46px sans-serif';ctx.fillText(lines[1]||'',450,145);
 ctx.fillStyle='#9de5ff';ctx.font='bold 38px sans-serif';ctx.fillText(lines[2]||'',450,215);
 ctx.fillStyle='#d7f7de';ctx.font='bold 30px sans-serif';ctx.fillText(lines[3]||'',450,260);
 ...
 const sprite=new THREE.Sprite(material);sprite.scale.set(4.8,1.6,1);sprite.renderOrder=9999;
 sprite.userData.disposeShortcutFeedback=()=>{texture.dispose();material.dispose();};
 return sprite;
}
...
function showShortcutFeedback(entry){
 ...
 if(activeShortcutFeedback){scene.remove(activeShortcutFeedback);activeShortcutFeedback.userData.disposeShortcutFeedback?.();activeShortcutFeedback=null;}
 clearTimeout(activeShortcutFeedbackTimer);
 const sprite=makeShortcutFeedbackSprite([`⌨ ${info.key}`,info.title,`${info.io} 觸發｜${info.functionName}`,info.item.name||'設備']);
 const box=new THREE.Box3().setFromObject(info.item.mesh);const center=new THREE.Vector3();box.getCenter(center);
 center.y=(Number.isFinite(box.max.y)?box.max.y:center.y)+1.1;sprite.position.copy(center);scene.add(sprite);activeShortcutFeedback=sprite;
 const status=$('statusText');if(status)status.textContent=`快捷鍵 ${info.key}｜${info.title}｜${info.io} ${info.functionName}｜${info.item.name}`;
 activeShortcutFeedbackTimer=setTimeout(()=>{if(activeShortcutFeedback===sprite){scene.remove(sprite);sprite.userData.disposeShortcutFeedback?.();activeShortcutFeedback=null;}},3600);
}
function renderShortcutList(){
 const box=$('shortcutList');if(!box)return;
 box.innerHTML=state.shortcuts.length?state.shortcuts.map((entry,index)=>`<article class="shortcut-row"><kbd>${shortcutComboLabel(entry.combo||entry.key)}</kbd><div><strong class="shortcut-title">${entry.title||entry.itemName||'未命名項目'}</strong><span class="shortcut-detail">${shortcutDescription(entry)}</span><small>${entry.ioType==='di'?'直接執行設備輸入動作':'模擬 DO 訊號脈衝並送往已接線設備'}</small></div><button data-shortcut-remove="${index}" type="button">刪除</button></article>`).join(''):'<div class="cloud-empty">尚未設定快捷鍵。</div>';
 box.querySelectorAll('[data-shortcut-remove]').forEach(btn=>btn.onclick=()=>{state.shortcuts.splice(Number(btn.dataset.shortcutRemove),1);renderShortcutList();markDirty();});
}
function openShortcutDialog(){renderShortcutDeviceOptions();renderShortcutList();$('shortcutDialog')?.showModal();}
function pulseShortcutDo(item,port){
 let count=0;
 for(const w of state.wires){
  if(w.sourceId!==item.id||w.sourcePort!==port)continue;
  const target=get(w.targetId);if(!target)continue;
  count++;state.activeWires.add(w.id);input(target,w.targetPort);
  setTimeout(()=>{state.activeWires.delete(w.id);drawWires();},450);
 }
 drawWires();
...
function executeShortcut(entry){
 ...
 window.UTOP_DEBUG?.record?.('ACTION','Shortcut',`快捷鍵 ${shortcutComboLabel(entry.combo||entry.key)}：${shortcutDescription(entry)}`);
}
$('shortcutBtn')?.addEventListener('click',openShortcutDialog);
$('closeShortcutDialog')?.addEventListener('click',()=>$('shortcutDialog')?.close());
$('shortcutDevice')?.addEventListener('change',renderShortcutPortOptions);
$('shortcutIoType')?.addEventListener('change',renderShortcutPortOptions);
$('shortcutKey')?.addEventListener('keydown',event=>{
 event.preventDefault();event.stopPropagation();
 const combo=shortcutSignatureFromEvent(event);
 if(!combo)return;
 event.currentTarget.value=shortcutComboLabel(combo);
 event.currentTarget.dataset.key=combo;
});
...
$('shortcutAdd')?.addEventListener('click',()=>{
 ...
 if(!key){alert('請先點「鍵盤按鍵」欄位，再按下要指定的鍵。');return;}
 if(!title){alert('請輸入「項目名稱」，例如：入口車輛進入。');return;}
 if(!deviceId||!port){alert('請選擇設備與 DI／DO 功能。');return;}
 state.shortcuts=state.shortcuts.filter(x=>(x.combo||x.key)!==key);
 state.shortcuts.push({combo:key,key,title,deviceId,ioType,port});renderShortcutList();markDirty();
 $('shortcutKey').value='';$('shortcutKey').dataset.key='';if($('shortcutTitle'))$('shortcutTitle').value='';
});
document.addEventListener('keydown',event=>{
 ...
 const combo=shortcutSignatureFromEvent(event);if(!combo)return;
 const entry=state.shortcuts.find(x=>(x.combo||x.key)===combo);if(!entry)return;
 event.preventDefault();event.stopPropagation();executeShortcut(entry);
});
