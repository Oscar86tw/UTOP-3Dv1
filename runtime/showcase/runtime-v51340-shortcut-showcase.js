...
function applyInspectorTransformPatch(item,patch,reason){
 ...
 writeWorldTransform(item,next,{refresh:false,record:false,source:reason});
 item.params=item.params||{};
...
/* =========================================================
   V5.1.3.40 展示快捷鍵管理：組合鍵／編輯／鎖定／浮動提示／3D發光
========================================================= */
...
function makeShortcutFeedbackSprite(info){
 const canvas=document.createElement('canvas');canvas.width=820;canvas.height=230;
 const ctx=canvas.getContext('2d');
 const isDo=info.io==='DO';
 const accent=isDo?'#ffbe55':'#56d8ff';
 const accentSoft=isDo?'rgba(255,190,85,.26)':'rgba(86,216,255,.25)';
 ctx.clearRect(0,0,canvas.width,canvas.height);
 const x=12,y=12,w=796,h=206,r=34;
 ctx.shadowColor=accent;ctx.shadowBlur=28;
 ctx.fillStyle='rgba(4,18,29,.92)';ctx.strokeStyle=accent;ctx.lineWidth=6;
 ...
 ctx.shadowBlur=0;
 ctx.fillStyle=accentSoft;ctx.beginPath();ctx.roundRect(260,132,300,58,24);ctx.fill();
 ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillStyle='#ffffff';ctx.font='900 58px sans-serif';ctx.fillText(info.title,410,78);
 ctx.fillStyle=accent;ctx.font='900 34px sans-serif';ctx.fillText(`${info.io}｜${info.functionName}`,410,161);
 ...
 const sprite=new THREE.Sprite(material);sprite.scale.set(4.7,1.32,1);sprite.renderOrder=9999;
...
function stopShortcutGlow(){
 ...
 if(session.halo){scene.remove(session.halo);session.halo.material?.map?.dispose?.();session.halo.material?.dispose?.();}
 if(session.light)scene.remove(session.light);
}
function createShortcutHalo(center){
 const c=document.createElement('canvas');c.width=256;c.height=256;const x=c.getContext('2d');
 const g=x.createRadialGradient(128,128,16,128,128,124);g.addColorStop(0,'rgba(255,255,255,.95)');g.addColorStop(.22,'rgba(255,235,90,.75)');g.addColorStop(.52,'rgba(80,220,255,.38)');g.addColorStop(1,'rgba(255,255,255,0)');x.fillStyle=g;x.fillRect(0,0,256,256);
 const tex=new THREE.CanvasTexture(c);const mat=new THREE.SpriteMaterial({map:tex,transparent:true,blending:THREE.AdditiveBlending,depthWrite:false,depthTest:false,opacity:.85});
 const sp=new THREE.Sprite(mat);sp.position.copy(center);sp.scale.set(3.2,3.2,1);sp.renderOrder=9997;scene.add(sp);return sp;
}
function startShortcutGlow(item){
 stopShortcutGlow();if(!item?.mesh)return;
 const box=new THREE.Box3().setFromObject(item.mesh),center=new THREE.Vector3();box.getCenter(center);
 const size=new THREE.Vector3();box.getSize(size);const scale=Math.max(size.x,size.y,size.z,1);
 const materials=[];
 item.mesh.traverse(obj=>{
  if(!obj.isMesh||!obj.material)return;
  const original=obj.material;const originals=Array.isArray(original)?original:[original];
  const clones=originals.map(mat=>{const m=mat.clone();if('emissive' in m){m.emissive=new THREE.Color(0xffd84d);m.emissiveIntensity=.3;}return m;});
  obj.material=Array.isArray(original)?clones:clones[0];materials.push({mesh:obj,original,clones});
 });
 const halo=createShortcutHalo(center);halo.scale.set(scale*2.4,scale*2.4,1);
 const light=new THREE.PointLight(0xffe65b,4,Math.max(scale*8,8),2);light.position.copy(center);scene.add(light);
 const session={materials,halo,light,raf:0,start:performance.now(),duration:3000};shortcutGlowSession=session;
 const animate=(now)=>{
  if(shortcutGlowSession!==session)return;
  const t=(now-session.start)/1000;const elapsed=now-session.start;
  const pulse=.5+.5*Math.sin(t*18);const color=new THREE.Color().setHSL((t*.55)%1,.95,.58+.12*pulse);
  for(const rec of materials)for(const mat of rec.clones){if('emissive' in mat){mat.emissive.copy(color);mat.emissiveIntensity=1.4+2.2*pulse;}}
  light.color.copy(color);light.intensity=2.5+5*pulse;halo.material.opacity=.46+.48*pulse;const hs=scale*(2.05+.55*pulse);halo.scale.set(hs,hs,1);
...
function resetShortcutEditor(){
 shortcutEditingIndex=-1;if($('shortcutKey')){$('shortcutKey').value='';$('shortcutKey').dataset.key='';}if($('shortcutTitle'))$('shortcutTitle').value='';
...
function editShortcut(index){
 ...
 $('shortcutKey').dataset.key=entry.combo||entry.key||'';$('shortcutKey').value=shortcutComboLabel(entry.combo||entry.key||'');$('shortcutTitle').value=entry.title||entry.itemName||'';
...
function showShortcutHover(entry,event){
 ...
 card.innerHTML=`<strong>${shortcutEscape(info.title)}</strong><span>⌨ ${shortcutEscape(info.key)}</span><span>${shortcutEscape(info.item?.name||'設備已刪除')}</span><span class="io ${String(entry.ioType).toLowerCase()}">${shortcutEscape(info.io)}｜${shortcutEscape(info.functionName)}</span><small>${entry.locked?'🔒 已鎖定：避免誤改':'🔓 可修改'}</small>`;
...
function executeShortcut(entry){const item=get(entry.deviceId);if(!item)return;if(entry.ioType==='do')pulseShortcutDo(item,entry.port);else input(item,entry.port);showShortcutFeedback(entry);window.UTOP_DEBUG?.record?.('ACTION','Shortcut',`${entry.title||'快捷觸發'}｜${String(entry.ioType).toUpperCase()} ${entry.port}`);}
...
$('shortcutAdd')?.addEventListener('click',()=>{
 const key=$('shortcutKey')?.dataset.key||'';const title=String($('shortcutTitle')?.value||'').trim();const deviceId=$('shortcutDevice')?.value||'';const ioType=$('shortcutIoType')?.value||'di';const port=$('shortcutPort')?.value||'';
 ...
 const next={combo:key,key,title,deviceId,ioType,port,locked:false};
