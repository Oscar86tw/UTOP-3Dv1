// UTOP-3D V5.1.3.40 - Fast Projects index open + manual Drive resync
const SPREADSHEET_ID = '1lmEqbYals_uBIOrQFDupYR2hs0iks5zEJM6FTOuOwP0';
const CONFIG_SHEET_NAME = '工作表1';
const EXEC_CACHE_KEY = 'utop3d_google_exec_url_v51340';
const LAST_FILE_NAME_KEY = 'utop3d_last_google_file_name';
const CLOUD_LOCKS_KEY = 'utop3d_cloud_project_locks_v51340';
...
let resolvedExecUrlPromise = null;

async function fetchAppsScriptUrlFromSheet() {
  const sheet = encodeURIComponent(CONFIG_SHEET_NAME);
  const csvUrl =
    `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}` +
    `/gviz/tq?tqx=out:csv&sheet=${sheet}&range=A2&t=${Date.now()}`;

  const response = await fetch(csvUrl, { method: 'GET', cache: 'no-store' });
  if (!response.ok) {
    throw new Error('無法讀取 Google Sheet「工作表1!A2」，請確認試算表分享權限');
  }

  const text = await response.text();
  const match = text.match(/https:\/\/script\.google\.com\/macros\/s\/[A-Za-z0-9_-]+\/exec/);
  if (!match) {
    throw new Error('工作表1!A2 找不到有效的 Apps Script /exec 網址');
  }
  return match[0];
}

async function resolveAppsScriptUrl(forceRefresh = false) {
  if (forceRefresh || !resolvedExecUrlPromise) {
    resolvedExecUrlPromise = (async () => {
      try {
        const url = await fetchAppsScriptUrlFromSheet();
        localStorage.setItem(EXEC_CACHE_KEY, url);
        return url;
      } catch (error) {
        const cached = localStorage.getItem(EXEC_CACHE_KEY);
        if (cached && /https:\/\/script\.google\.com\/macros\/s\/[A-Za-z0-9_-]+\/exec/.test(cached)) {
          console.warn('[UTOP-3D] A2 讀取失敗，暫時使用最後一次已驗證端點。', error);
          showToast('A2 暫時讀取失敗，使用最後一次雲端端點；請檢查工作表1!A2。', true);
          return cached;
        }
        throw error;
      }
...
async function saveGoogleProject() {
  ...
  try {
    ...
    const result = await postBridge('saveProject', {
      ...
      project,
      data: project,
      version: project?.version || '5.1.3.40'
    });
...
async function createGoogleSaveFolder() {
  const input = $('googleSaveNewFolderName');
  const name = input?.value.trim();
  if (!name) {
    showToast('請輸入資料夾名稱', true);
    input?.focus();
    return;
  }

  const button = $('createGoogleSaveFolder');
  if (button) {
    button.disabled = true;
  ...
  try {
    const parentFolderId = $('googleFolderSelect')?.value || '';
    const result = await postBridge('createFolder', {
      folderName: name,
      parentFolderId
    });
    ...
    if ($('googleFolderSelect') && result.folderId) {
      $('googleFolderSelect').value = result.folderId;
    }

    const box = $('googleSaveFolderCreateBox');
    if (box) box.hidden = true;
    showToast(`已新增並選取資料夾：${result.folderName || name}`);
  } catch (error) {
    showToast(`新增資料夾失敗：${error.message}`, true);
  } finally {
    if (button) {
      button.disabled = false;
      button.textContent = '建立並選取';
    }
  }
}
...
async function refreshGoogleProjects(options = {}) {
  $('googleProjectList').innerHTML = '<div class="cloud-empty">正在讀取雲端專案索引…</div>';
  try {
    // 快速模式：本次頁面第一次使用才讀 A2；後續沿用已驗證端點。
    if (options.forceEndpoint === true) await resolveAppsScriptUrl(true);
    else await resolveAppsScriptUrl(false);

    await refreshGoogleFolders();
    // 一次讀完整 Projects 索引，資料夾與搜尋都在前端即時篩選。
    ...
    showToast(`雲端索引讀取失敗：${error.message}`, true);
  }
}

async function syncGoogleDriveIndex() {
  const button = $('syncGoogleDriveIndex');
  if (button) { button.disabled = true; button.textContent = '同步中…'; }
  $('googleProjectList').innerHTML = '<div class="cloud-empty">正在完整掃描 Google Drive，第一次或檔案很多時會較久…</div>';
  try {
    await resolveAppsScriptUrl(true);
    const syncResult = await postBridge('syncDriveIndex', {
      recursive: true,
      createFolderIndex: true,
      markMissing: true
    });
    await refreshGoogleProjects();
    const added = Number(syncResult?.addedProjects || 0);
    const folders = Number(syncResult?.addedFolders || 0);
    const missing = Number(syncResult?.missingProjects || 0);
    showToast(`Drive 同步完成：新增專案 ${added}、資料夾 ${folders}${missing ? `、缺檔 ${missing}` : ''}`);
  } catch (error) {
    showToast(`Drive 同步失敗：${error.message}`, true);
    await refreshGoogleProjects().catch(()=>{});
  } finally {
    if (button) { button.disabled = false; button.textContent = '🔄 重新同步 Drive'; }
  }
}
...
function ensureLegacyImportPanel() {
  ...
  panel.innerHTML = `
    ...
    <button id="legacyCloudImportBtn" type="button">匯入 Drive 實體檔</button>
  `;
...
async function checkLegacyCloudProjects() {
  ...
  try {
    ...
    if (count > 0 && importable > 0) {
      panel.hidden = false;
      $('legacyCloudImportText').textContent = `工作表1偵測到 ${count} 筆舊資料，其中 ${importable} 筆可匯入；新版 Projects 目前 ${projectCount} 筆。匯入後實體 JSON 會存到 Google Drive；工作表1仍保留。`;
      const btn = $('legacyCloudImportBtn');
      btn.disabled = false;
      btn.textContent = '匯入 Drive 實體檔';
    } else {
      panel.hidden = true;
    }
  } catch (error) {
...
function bindStorageCenter() {
  ...
  if ($('googleOpenFolderFilter')) $('googleOpenFolderFilter').onchange = renderProjectList;
  if ($('syncGoogleDriveIndex')) $('syncGoogleDriveIndex').onclick = syncGoogleDriveIndex;
  if ($('createGoogleFolder')) $('createGoogleFolder').onclick = createGoogleFolder;
  if ($('toggleGoogleSaveFolderCreate')) $('toggleGoogleSaveFolderCreate').onclick = () => {
    const box = $('googleSaveFolderCreateBox');
    if (!box) return;
    box.hidden = !box.hidden;
    if (!box.hidden) $('googleSaveNewFolderName')?.focus();
  };
  if ($('cancelGoogleSaveFolderCreate')) $('cancelGoogleSaveFolderCreate').onclick = () => {
    const box = $('googleSaveFolderCreateBox');
    if (box) box.hidden = true;
    if ($('googleSaveNewFolderName')) $('googleSaveNewFolderName').value = '';
  };
  if ($('createGoogleSaveFolder')) $('createGoogleSaveFolder').onclick = createGoogleSaveFolder;
  if ($('googleSaveNewFolderName')) $('googleSaveNewFolderName').onkeydown = (event) => {
    if (event.key === 'Enter') { event.preventDefault(); createGoogleSaveFolder(); }
    if (event.key === 'Escape') { event.preventDefault(); const box = $('googleSaveFolderCreateBox'); if (box) box.hidden = true; }
  };
