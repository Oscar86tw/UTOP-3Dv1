...
loopdetector:{name:'地感檢知器',assetId:'LANE-LOOP-DETECTOR-001',defaults:{rotation:0,mode:'PRESENCE',vehicle:false,pulse:false,fault:false,ready:true,sensitivity:4,frequency:'MID',contact:'NO',showLabel:true},di:[['DI1','LOOP_INPUT｜線圈偵測','vehicle'],['DI2','CLEAR｜車輛離開','clear'],['DI3','TEST_PULSE｜脈衝測試','pulse'],['DI4','LOOP_FAULT｜線圈故障','fault'],['DI5','RESET｜重新校正','reset']],do:[['DO1','PRESENCE_OUT｜車輛存在','presence'],['DO2','PULSE_OUT｜脈衝輸出','pulseOut'],['DO3','LOOP_FAULT｜線圈故障','faultOut'],['DO4','READY｜設備正常','ready']]},
 ...
 object_car:{name:'汽車',assetId:'OBJECT-CAR-001',defaults:{rotation:0,showLabel:true,objectKind:'car',realDimensions:{width:1.8,height:1.45,depth:4.4}},di:[],do:[]},
 object_motorcycle:{name:'摩托車',assetId:'OBJECT-MOTORCYCLE-001',defaults:{rotation:0,showLabel:true,objectKind:'motorcycle',realDimensions:{width:.8,height:1.25,depth:2.1}},di:[],do:[]},
...
function renderInspector(){
 ...
 if(state.activeTab==='properties'){
   ...
   if(i.type==='barrier'){
    ...
    box.innerHTML=`<div class="form-grid"><section class="field-group"><h3>場景物件</h3><label>物件種類<input value="${d.name}" disabled></label><label>用途<input value="非弱電設備／場景建模" disabled></label><small>尺寸請至「尺寸」頁籤調整；位置、角度與高度在「屬性」調整。</small></section></div>`;
   }else{
...
/* =========================================================
   V5.1.3.41 展示快捷鍵管理：組合鍵／編輯／鎖定／浮動提示／3D發光
========================================================= */
...
function showShortcutFeedback(entry){
 ...
 activeShortcutFeedbackTimer=setTimeout(()=>{if(activeShortcutFeedback===sprite){scene.remove(sprite);sprite.userData.disposeShortcutFeedback?.();activeShortcutFeedback=null;}},3000);
}
...
function showShortcutHover(entry,event){
 ...
 card.innerHTML=`<strong>${shortcutEscape(info.title)}</strong><span>⌨ ${shortcutEscape(info.key)}</span><span>${shortcutEscape(info.item?.name||'設備已刪除')}</span><span class="io ${String(entry.ioType).toLowerCase()}">${shortcutEscape(info.io)}｜${shortcutEscape(info.functionName)}</span><span>⏱ 延遲 ${Math.max(0,Number(entry.delaySeconds??5)||0)} 秒</span><small>${entry.locked?'🔒 已鎖定：避免誤改':'🔓 可修改'}</small>`;
...
const shortcutDelaySessions=new Map();
function executeShortcut(entry){
 const item=get(entry.deviceId);if(!item)return;
 const delay=Math.max(0,Math.min(60,Number(entry.delaySeconds??5)||0));
 const sessionKey=`${entry.combo||entry.key||''}|${entry.deviceId}|${entry.ioType}|${entry.port}`;
 const previous=shortcutDelaySessions.get(sessionKey);if(previous){clearInterval(previous.interval);clearTimeout(previous.timer);shortcutDelaySessions.delete(sessionKey);}
 const run=()=>{shortcutDelaySessions.delete(sessionKey);const latest=get(entry.deviceId);if(!latest)return;if(entry.ioType==='do')pulseShortcutDo(latest,entry.port);else input(latest,entry.port);showShortcutFeedback(entry);window.UTOP_DEBUG?.record?.('ACTION','Shortcut',`${entry.title||'快捷觸發'}｜${String(entry.ioType).toUpperCase()} ${entry.port}｜延遲 ${delay} 秒`);};
 if(delay<=0){run();return;}
 let remain=Math.ceil(delay);const status=$('statusText');if(status)status.textContent=`${entry.title||'快捷觸發'}｜${remain} 秒後執行`;
 const interval=setInterval(()=>{remain-=1;if(remain>0&&status)status.textContent=`${entry.title||'快捷觸發'}｜${remain} 秒後執行`;},1000);
 const timer=setTimeout(()=>{clearInterval(interval);run();},delay*1000);shortcutDelaySessions.set(sessionKey,{interval,timer});
}
