...
function makeModel(item){
 ...
 if(item.type==='barrier'){
   ...
   if(kind==='column'){
     ...
     const body=new THREE.Mesh(new THREE.BoxGeometry(w*.96,h*.42,d*.92),mat(0x3f68a8,{roughness:.32,metalness:.45}));body.position.y=h*.32;g.add(body);
     const cabin=new THREE.Mesh(new THREE.BoxGeometry(w*.78,h*.42,d*.46),mat(0x5c7da8,{roughness:.2,metalness:.25}));cabin.position.set(0,h*.68,-d*.05);g.add(cabin);
     [[-.42,-.34],[.42,-.34],[-.42,.34],[.42,.34]].forEach(([sx,sz])=>{const wheel=new THREE.Mesh(new THREE.CylinderGeometry(h*.14,h*.14,w*.12,20),mat(0x17191b,{roughness:.85}));wheel.rotation.z=Math.PI/2;wheel.position.set(sx*w, h*.16, sz*d);g.add(wheel)});
   }else if(kind==='motorcycle'){
     [-d*.34,d*.34].forEach(z=>{const wheel=new THREE.Mesh(new THREE.TorusGeometry(h*.22,.055,10,22),mat(0x181a1c,{roughness:.8}));wheel.rotation.y=Math.PI/2;wheel.position.set(0,h*.25,z);g.add(wheel)});
     const frame=new THREE.Mesh(new THREE.BoxGeometry(w*.25,h*.45,d*.55),mat(0xb82424,{roughness:.38,metalness:.35}));frame.position.y=h*.48;g.add(frame);
   }else if(kind==='person'){
...
/* V5.1.3.44：汽車／摩托車可駕駛、速度與方向控制；地感線圈／紅外線自動感應觸發。 */
