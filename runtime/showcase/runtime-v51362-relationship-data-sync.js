...
function buildRelationshipSnapshot(){
 ...
 return {version:'5.1.3.62',generatedAt:new Date().toISOString(),projectName:document.title||'UTOP-3D',items,wires:state.wires.map(w=>({...w})),shortcuts:(state.shortcuts||[]).map(x=>({...x}))};
}
...
function openRelationshipPage(path){
 ...
 if(!win){alert('瀏覽器阻擋了新分頁，請允許此網站開啟分頁。');return;}
 setTimeout(()=>{
  try{win.postMessage({type:'UTOP_RELATIONSHIP_SNAPSHOT',snapshot},location.origin);}catch{}
 },250);
}
