...
function renderInspector(){
 ...
 if(state.activeTab==='properties'){
   ...
   <label class="checkbox-field"><input id="showModuleStand" type="checkbox" ${p.showStand!==false?'checked':''}> 顯示設備支架（有支架的模組）</label>
...
if(OBJECT_TYPES.has(i.type))return`場景物件：${defs[i.type]?.name||i.type}<br>實體碰撞：${scenePhysicsSettings.collisionEnabled!==false?'開啟':'關閉'}`;
...
function bindInspector(i){
 ...
 if($('showModuleStand'))$('showModuleStand').onchange=e=>{
  ...
  rebuildItemGeometryOnly(i,{reason:'stand-visibility'});
  applyPartsVisibility(i);
...
function constrainWorldTransform(item,before,next,options={}){
 ...
 if(scenePhysicsSettings.collisionEnabled!==false && options.ignoreCollision!==true){
